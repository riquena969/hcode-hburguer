...
export class OrderService {
  ...
  async create(createOrderDto: CreateOrderDto) {
    ...
    const products = await Promise.all(createOrderDto.products.map(async (product: CreateOrderProductDto) => {
      ...
      const itens = await Promise.all(product.itens.map(async (itemId: number) => {
      ...
      productValue += itens.reduce((prev: number, cur: number) => { return prev + cur });
    ...
    const totalValue = products.reduce((prev: number, cur: number) => { return prev + cur });

    return this.prismaService.orders.update({ where: { id: order.id }, data: { value: totalValue } });
  }
